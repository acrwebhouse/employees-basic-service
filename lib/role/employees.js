...
function editManyEmployees(editEmployees,companyId,userId,rank,managerId,state,isResign, callback) {
    ...
    if(editEmployees.companyId && editEmployees.companyId.length == 24){
        match.companyId = ObjectId(editEmployees.companyId)
    }

    if(editEmployees.userId && editEmployees.userId.length == 24){
        match.userId = ObjectId(editEmployees.userId)
    }

    if(editEmployees.managerId && editEmployees.managerId.length == 24){
        match.managerId = ObjectId(editEmployees.managerId)
    }

    if(utilsValue.isValid(editEmployees.ids) && editEmployees.ids.length > 0){
        const ids = []
        for(let i = 0 ;i<editEmployees.ids.length;i++){
            ids.push(ObjectId(editEmployees.ids[i]))
        }
        match._id = { $in: ids}
    }
    ...
    mongoDB.updateMany(collectionName, match, updateData, (result,data)=>{
        if(result && data.nModified>0){
            callback(true,data)
        }else{
            callback(false,data)
